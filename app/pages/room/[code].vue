...
const connectionTone = computed(() => {
  switch (connectionState.value) {
    case "connected":
      return "bg-[rgba(74,164,111,0.18)] text-[#b8f0cb]";
    case "connecting":
      return "bg-[rgba(200,165,87,0.16)] text-[#f5d79a]";
    case "error":
      return "bg-[rgba(211,104,104,0.18)] text-[#ffb8b8]";
    default:
      return "bg-white/5 text-[rgba(231,243,235,0.82)]";
  }
});
...
const panelClass =
  "rounded-[24px] border border-[rgba(179,224,193,0.12)] bg-[rgba(6,18,12,0.72)] p-5 backdrop-blur-[18px]";
const eyebrowClass =
  "mb-2 text-xs uppercase tracking-[0.24em] text-caro-accent";
const ghostButtonClass =
  "rounded-xl border border-[rgba(179,224,193,0.12)] bg-transparent px-5 py-2.5 font-semibold text-white transition duration-200 hover:-translate-y-px hover:opacity-90";
const statusCardClass =
  "flex items-center gap-2 md:gap-4 rounded-[18px] border border-[rgba(179,224,193,0.12)] bg-[rgba(6,18,12,0.72)] px-3 md:px-[18px] py-2 md:py-3 backdrop-blur-[18px]";

const canRestart = computed(
  () => connectionState.value === "connected" && myRole.value === "host",
);
...
const winnerInfo = computed(() => {
  ...
  const winnerPlayer = snapshot.value.players.find(
    (p) => p.role === (winnerMark === "X" ? "host" : "guest"),
  );
  ...
  return {
    ...
    message: isMe
      ? "Tuyệt vời, bạn đã đánh bại đối thủ!"
      : `${winnerName} đã giành chiến thắng ván này.`,
...
const gameRules = computed(() => {
  const type = snapshot.value?.gameType || "caro";
  if (type === "chess") {
    return [
      "Host là quân Trắng (đi trước), Guest là quân Đen.",
      "Sử dụng luật Cờ Vua thi đấu quốc tế.",
      "Chiếu cháy (Checkmate) hoặc Hết nước đi (Draw/Stalemate).",
      "Tổng thời gian mỗi ván: 3 phút.",
    ];
  }
  return [
    "Host là X, Guest là O.",
    "Ai có 5 quân liên tiếp trước sẽ thắng.",
    "Tổng thời gian mỗi ván: 3 phút.",
    "Hết giờ người đang lượt đi sẽ thua.",
  ];
});
</script>
...
<template>
  <div v-if="isVerified" class="space-y-6 pb-12">
    ...
    <section class="flex flex-wrap items-center justify-between gap-4">
      <div class="flex flex-wrap gap-3">
        <div :class="statusCardClass">
          <span class="text-xs uppercase text-[rgba(231,243,235,0.62)]"
            >Phòng</span
          >
          <strong
            class="text-caro-accent tracking-widest font-mono text-base leading-4 md:text-lg"
            >{{ roomCode }}</strong
          >
        </div>
        <div :class="statusCardClass">
          ...
          >
          <strong
            :class="['rounded-full px-3 py-1 text-[0.9rem]', connectionTone]"
            >{{ connectionLabel }}</strong
          >
        </div>
        <button :class="ghostButtonClass" @click="handleLeave">
          Rời phòng
        </button>
      </div>
    </section>
    ...
    <!-- Main Game Grid -->
    <section
      class="grid gap-4 sm:gap-6 xl:grid-cols-[350px_minmax(0,1fr)_320px] xl:items-start"
    >
      <!-- Left Column: Score & Info -->
      <aside class="order-1 xl:order-0 grid gap-4 sm:gap-5">
        <CaroScoreboard
          v-if="snapshot"
          :snapshot="snapshot"
          :myRole="myRole"
        ...
        <div v-if="notice" :class="panelClass">
          <div
            class="rounded-[10px] border border-[rgba(200,165,87,0.2)] bg-[rgba(200,165,87,0.1)] p-2.5 text-[0.85rem] sm:text-[0.9rem] text-[#f5d79a]"
          >
            {{ notice }}
          </div>
        </div>
        ...
        <div :class="[panelClass, 'hidden sm:block']">
          ...
          <ul
            class="list-disc space-y-1.5 pl-[18px] text-[0.85rem] text-[rgba(231,243,235,0.7)]"
          >
      ...
      <!-- Center: The Board -->
      <div class="order-2 xl:order-0 overflow-hidden">
        <div v-if="snapshot">
          <ChessBoard
            v-if="snapshot.gameType === 'chess'"
            :snapshot="snapshot"
            :myMark="myMark"
            :isLoading="isLoading"
            @move="playChessMove"
          />
          <XiangqiBoard
            v-else-if="snapshot.gameType === 'xiangqi'"
            :snapshot="snapshot"
            :myMark="myMark"
            :isLoading="isLoading"
            @move="playChessMove"
        ...
        >
          <div
            class="h-12 w-12 animate-spin rounded-full border-4 border-white/10 border-t-caro-accent mb-4"
          />
    ...
    <!-- Winner Dialog -->
    <BaseDialog
      :show="showWinnerDialog"
      :title="winnerInfo?.title || 'Kết quả'"
      @close="showWinnerDialog = false"
    >
      <div v-if="winnerInfo" class="space-y-4 text-center py-4">
        <div
          class="inline-flex h-20 w-20 items-center justify-center rounded-3xl bg-white/5 mb-2"
        >
          <span :class="['text-5xl font-bold', winnerInfo.color]">
            {{ snapshot?.winner === "draw" ? "=" : snapshot?.winner }}
          </span>
        </div>
        <h3 :class="['text-2xl font-bold', winnerInfo.color]">
          {{ winnerInfo.title }}
        </h3>
      ...
      <template #footer>
        ...
        <button
          v-if="canRestart"
          :class="[
            'rounded-xl bg-caro-accent px-6 py-2.5 font-bold text-caro-bg-deep transition',
          ]"
      ...
      <div class="space-y-4 text-center py-4">
        ...
        <p class="text-[rgba(231,243,235,0.7)]">
          ...
          <span
            ...
            class="block mt-2 text-amber-400"
            >Tiến trình hiện tại sẽ bị hủy bỏ.</span
          >
        </p>
      </div>

      <template #footer>
        <button :class="ghostButtonClass" @click="showRestartConfirm = false">
          Hủy
        </button>
        <button
          :class="[
