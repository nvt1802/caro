...
const eyebrowClass = 'mb-2 text-xs uppercase tracking-[0.24em] text-caro-accent font-bold'
</script>
  ...
  <main class="relative mx-auto min-h-screen max-w-[1440px] px-5 pb-10 pt-8 font-caro text-caro-text">
    ...
    <header class="mb-10 flex items-center justify-between">
      <div @click="navigateTo('/')" class="cursor-pointer">
        <h1 class="text-2xl font-black italic tracking-tighter text-white uppercase">Caro<span
            class="text-caro-accent">Online</span></h1>
      </div>

      <nav class="hidden sm:flex gap-6 text-sm font-bold uppercase tracking-widest text-[rgba(231,243,235,0.6)]">
        <NuxtLink to="/" class="hover:text-caro-accent transition" active-class="text-caro-accent">Trang chủ</NuxtLink>
        <NuxtLink to="/lobby" class="hover:text-caro-accent transition" active-class="text-caro-accent">Sảnh chờ
        </NuxtLink>
      </nav>
    </header>
    ...
    <Teleport to="body">
      <div v-if="isLoading && !loadingCell"
        ...
        <div class="flex flex-col items-center gap-4">
          ...
          <p class="text-sm font-medium tracking-widest text-[#b8f0cb] uppercase italic">Đang đồng bộ...</p>
        </div>
      </div>
    </Teleport>
  </main>
